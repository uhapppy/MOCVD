...
class Result(QtWidgets.QWidget):
    def __init__(self, solution, wavelength_mat, n_mat, k_mat, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi(os.path.join(os.path.dirname(__file__), 'Result.ui'), self)
        ...
        self.wavelength = np.array(wavelength_mat)
    ...
    def table_index(self):
        ...
        for i in range(0, len(self.wavelength)):
            if self.unit_combobox.currentIndex() == 0:
                ...
                item = QtWidgets.QTableWidgetItem("{:.6f}".format(self.wavelength[i]))
                self.index_table.setItem(i, 0, item)
            elif self.unit_combobox.currentIndex() == 1:

                self.index_table.setHorizontalHeaderLabels(["Energy (eV)", "n", "k"])
                item = QtWidgets.QTableWidgetItem("{:.6f}".format(self.energy[i]))
                self.index_table.setItem(i, 0, item)
    ...
    def nk_peak(self):
        ...
        pen_n = pg.mkPen(color="k", width=2, style=QtCore.Qt.PenStyle.SolidLine)
        pen_k = pg.mkPen(color="k", width=2, style=QtCore.Qt.PenStyle.DashLine)
    ...
    def nk_peak_sum(self):
        self.mat_graph = pg.PlotWidget()
        styles = {'color': 'k', 'font-size': '20px'}

        pen_n_fit = pg.mkPen(color=(255, 0, 0), width=2, style=QtCore.Qt.PenStyle.SolidLine)
        pen_k_fit = pg.mkPen(color=(255, 0, 0), width=2, style=QtCore.Qt.PenStyle.DashLine)

        pen_n_mat = pg.mkPen(color=(0, 0, 255), width=2, style=QtCore.Qt.PenStyle.SolidLine)
        pen_k_mat = pg.mkPen(color=(0, 0, 255), width=2, style=QtCore.Qt.PenStyle.DashLine)

        legend = pg.LegendItem(pen="k")

        n_fit_legend = pg.PlotDataItem(pen=pen_n_fit)
        k_fit_legend = pg.PlotDataItem(pen=pen_k_fit)
        n_mat_legend = pg.PlotDataItem(pen=pen_n_mat)
        k_mat_legend = pg.PlotDataItem(pen=pen_k_mat)
        ...
        if self.unit_combobox.currentIndex() == 0:
            x_axis_data = self.wavelength
            self.mat_graph.setLabel('bottom', 'Wavelength', units='m', **styles)
            self.mat_graph.getAxis('bottom').setScale(1e-9)
        elif self.unit_combobox.currentIndex() == 1:
            x_axis_data = self.energy
            self.mat_graph.setLabel('bottom', 'Energy', units='eV', **styles)

        if self.include_mat_check.isChecked():
            if self.include_n_check.isChecked():
                self.mat_graph.plot(x_axis_data, self.n, pen=pen_n_fit)
                self.mat_graph.plot(x_axis_data, self.n_mat, pen=pen_n_mat)
                legend.addItem(n_fit_legend, 'n fit')
                legend.addItem(n_mat_legend, 'n mat')
            if self.include_k_check.isChecked():
                self.mat_graph.plot(x_axis_data, self.k, pen=pen_k_fit)
                self.mat_graph.plot(x_axis_data, self.k_mat, pen=pen_k_mat)
                legend.addItem(k_fit_legend, 'k fit')
                legend.addItem(k_mat_legend, 'k mat')
        else:
            if self.include_n_check.isChecked():
                self.mat_graph.plot(x_axis_data, self.n, pen=pen_n_fit)
                legend.addItem(n_fit_legend, 'n fit')
        ...
        legend.setParentItem(self.mat_graph.graphicsItem())

        self.mat_graph.showGrid(x=True, y=True)
        self.mat_graph.setBackground('w')
        self.mat_graph.setTitle("sum of selected peak", color="k", size="20pt")
        self.mat_graph.setLabel('left', 'Index', units='a.u.', **styles)

        # self.graph_layout.addWidget(self.graph_nk_peak_sum)
        self.graph_tab_holder.addTab(self.mat_graph, "Index")
    ...
    def save_solution_spread(self):
        ...
        file.write(f"Thickness (nm) , {9999.999999} , {9999.999999} \n")
