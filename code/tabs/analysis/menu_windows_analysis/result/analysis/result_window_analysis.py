...
class ResultWindowAnalysis(QtWidgets.QWidget):
    def __init__(self, layer_list, solution, chisquare, wavelength_exp, reflectance_exp, *args, **kwargs):
        super().__init__(*args, **kwargs)
        uic.loadUi(os.path.join(os.path.dirname(__file__), 'ResultWindowAnalysis.ui'), self)
        ...
        self.wavelength = np.array(wavelength_exp)
        ...
        self.reflectance_solution = get_reflectance(self.solution, self.layer_list, self.wavelength)
            ...
            if layer.first_or_last == True:
                if layer.fb_term is not None:
                    self.result_tab_holder.addTab(
                        ResultFBTabAnalysis(layer, self.solution[solution_start:(solution_start + 2 + layer.fb_term * 3)],
                                            wavelength_exp), layer.name)
                    solution_start += (2 + layer.fb_term * 3)

                elif layer.fb_term is None:
                    self.result_tab_holder.addTab(ResultMATTabAnalysis(layer, 9999.999999, wavelength_exp),
                                                  layer.name)
                ...
                if layer.fb_term is not None:
                    self.result_tab_holder.addTab(
                        ResultFBTabAnalysis(layer, self.solution[solution_start:(solution_start + 3 + layer.fb_term * 3)],
                                            wavelength_exp), layer.name)
                    solution_start += (3 + layer.fb_term * 3)
                elif layer.fb_term is None:
                    self.result_tab_holder.addTab(ResultMATTabAnalysis(layer, self.solution[solution_start], wavelength_exp),
                                                  layer.name)
                    solution_start += 1
    ...
    def table(self):
        if self.unit_combobox.currentIndex() == 0:
            self.reflectance_table.setHorizontalHeaderLabels(['Wavelength (nm)', 'Experimental', 'Solution', 'Peaks'])
            for i in range(0, len(self.wavelength)):
                self.reflectance_table.setItem(i, 0, QtWidgets.QTableWidgetItem("{:.6f}".format(self.wavelength[i])))
                self.reflectance_table.setItem(i, 1, QtWidgets.QTableWidgetItem("{:.6f}".format(self.reflectance_exp[i])))
                self.reflectance_table.setItem(i, 2, QtWidgets.QTableWidgetItem("{:.6f}".format(self.reflectance_solution[i])))
                self.reflectance_table.setItem(i, 3, QtWidgets.QTableWidgetItem("{:.6f}".format(self.reflectance_peaks[i])))


        elif self.unit_combobox.currentIndex() == 1:
            self.reflectance_table.setHorizontalHeaderLabels(['Energy (eV)', 'Experimental', 'Solution', 'Peaks'])
    ...
    def save_table(self):
        ...
        if self.unit_combobox.currentIndex() == 0:
            ...
            for i in range(0, len(self.wavelength)):
                file.write(
                    f"{self.wavelength[i]} , {self.reflectance_exp[i]} , {self.reflectance_solution[i]} , {self.reflectance_peaks[i]} \n")
        elif self.unit_combobox.currentIndex() == 1:
            file.write(f"Energy (eV) , Experimental , Solution , Peaks  \n")
            for i in range(0, len(self.energy)):
                file.write(
                    f"{self.energy[i]} , {self.reflectance_exp[i]} , {self.reflectance_solution[i]} , {self.reflectance_peaks[i]} \n")
    ...
    def frame_color(self):
        ...
        interpolation = interp1d(self.wavelength, self.reflectance_exp, kind='quadratic', fill_value="extrapolate")
        ...
        self.exp_color_frame.setStyleSheet(f"background-color: {color}")

        interpolation = interp1d(self.wavelength, self.reflectance_solution, kind='quadratic', fill_value="extrapolate")
        wavelength= np.arange(350, 900, 1)
        reflectance = interpolation(wavelength)
        dict_reflectance = dict(zip(wavelength,reflectance ))
        distribution = colorimetry.SpectralDistribution(data = dict_reflectance)
        ...
        if max(RGBbulk > 1):  RGBbulk = RGBbulk / max(RGBbulk)
        color = colour.notation.RGB_to_HEX(RGBbulk)
        self.solution_color_frame.setStyleSheet(f"background-color: {color}")
    ...
    def graph_reflectance(self):
        ...
        if self.unit_combobox.currentIndex() == 0:
            self.graph.plot(self.wavelength, self.reflectance_exp, pen=pg.mkPen('g', width=2), name='Experimental')
            self.graph.plot(self.wavelength, self.reflectance_peaks, pen=pg.mkPen('b', width=2), name='Peaks')
            self.graph.plot(self.wavelength, self.reflectance_solution, pen=pg.mkPen('r', width=2), name='Solution')
        ...
        if self.unit_combobox.currentIndex() == 1:
            self.graph.plot(self.energy, self.reflectance_exp, pen=pg.mkPen('g', width=2), name='Experimental')
            self.graph.plot(self.energy, self.reflectance_peaks, pen=pg.mkPen('b', width=2), name='Peaks')
            self.graph.plot(self.energy, self.reflectance_solution, pen=pg.mkPen('r', width=2), name='Solution')
